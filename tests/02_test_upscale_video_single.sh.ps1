...
$null = I18N-Status-Print "note" "test single video upscale...`n"
$__process = . "${env:LIBS_UPSCALER}\start.ps1" `
                "--model" "upscayl-nmkd-4x-superscale-sd-178000-g" `
                "--scale" "4" `
                "--parallel" "2" `
                "--input" "${env:UPSCALER_PATH_ROOT}\tests\video\sample-1-640x360.mp4" `
                "--output" "${env:UPSCALER_PATH_ROOT}\tmp\test-01\sample-01-upscaled.mp4"
if ($__process -ne 0) {
        $null = I18N-Status-Print "error" "Failed - bad processing.`n`n"
        $__verdict = $false
}

if (-not (Test-Path -Path "${env:UPSCALER_PATH_ROOT}\tmp\test-01\sample-01-upscaled.mp4")) {
...
if [ ! -f "${UPSCALER_PATH_ROOT}/tmp/test-01/sample-01-upscaled.mp4" ]; then
